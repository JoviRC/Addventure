...
@Component({
  selector: 'app-g-two-lv-two',
  templateUrl: './g-two-lv-two.page.html',
  styleUrls: ['./g-two-lv-two.page.scss'],
})
export class GTwoLvTwoPage implements OnInit {
  ...
  fnNums() { // Rellena numOne y numTwo random y previene repeticiones con el juego anteriro.
    this.clearTimer()
    this.startTimer()
    this.gService.fnSaveContadorLevelTwoGameTwo()
  ...
  fnCurrentGame() {
    if (this.currentGame < 4) {
      ...
      setTimeout(() => {
        ...
        this.currentGame = 1
        location.replace('/g-two-lv-three')
      }, 2500);
    }
  }
  ...
  fnSelectBtnOne() { //comprueba btn 1
    if (this.arBtn[0] == this.numTotal) {
      ...
      setTimeout(() => {
        ...
        this.gService.fnSaveCoinsTwolvTwo()
        this.gService.fnSaveExpGameTwoLvTwo()
        this.playAudioBueno()
      }, 500);
      setTimeout(() => {
        var mod = document.getElementById('modalTrue')
